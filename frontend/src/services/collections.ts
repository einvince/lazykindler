...
export const deleteCollectionWithBooks = (uuid: string) => {
  return axiosInstance.delete(`/api/collection/delete/withitems?uuid=${uuid}`).then((data: any) => {
    return data.data;
  });
};
...
export const getCollBooks = (coll_uuid: string) => {
  return axiosInstance.get(`/api/collection/get/books?coll_uuid=${coll_uuid}`).then((data: any) => {
    return data.data;
  });
};
